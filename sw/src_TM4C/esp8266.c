...
#define READYLINE GPIO_PORTB_DATA_R

#define     LOW   0x0
#define     HIGH  0x1

#define RDYLINE GPIO_PORTB_DATA_R
...
#ifdef UTIOT
    char    eid[32]             = "erh2656";                    // Your EID goes here
    char    ssid[32]            = "utexas-iot";                  // WiFi Access Point in 5.826
    char    pass[32]            = "33940254006967049323";
    char    mqtt_broker[32]     = "10.159.177.113";              // MQTT Broker in EER
    char    mqtt_port[8]        = "1883";
#endif
...
void PortB_Init(void) {
    ...
    // Set up PB4 and PB5 as GPIO with digital functionality
    GPIO_PORTB_AMSEL_R &= ~0x30;             // Disable analog mode on PB4 and PB5
    GPIO_PORTB_AFSEL_R &= ~0x30;             // Disable alternate functions on PB4 and PB5
    GPIO_PORTB_PCTL_R &= ~0x00FF0000;        // Set PB4 and PB5 as GPIO
    GPIO_PORTB_DEN_R |= 0x30;                // Enable digital functionality on PB4 and PB5

    // Set directions: PB5 as output, PB4 as input
    GPIO_PORTB_DIR_R |= 0x20;                // Set PB5 as output
    GPIO_PORTB_DIR_R &= ~0x10;               // Set PB4 as input

    // Now, set PB5 high
    GPIO_PORTB_DATA_R |= 0x20;               // Set PB5 to high (1)
}
...
void Reset_8266(void) {
  ...
  DelayWait10ms(1500);           // Wait for 8266 to reset
  //PB5 = 0x20;                   // Enable the 8266
  GPIO_PORTB_DATA_R |= 0x20;
  DelayWait10ms(1500);           // Wait before setting up 8266
}
...
void SetupWiFi(void) {  
 ...
 // ST7735_DrawString(0,1,"Waiting for RDY flag ", ST7735_Color565(255, 255, 255 ));
  ...
  while (!(READYLINE & 0x10)) {      // Wait for ESP8266 indicate it is ready for programming data
    #ifdef DEBUG1
      UART_OutString(".");
    #endif
  ...
  UART_OutString(eid);           // Student EID - Used for individualizing MQTT Topics
  UART_OutChar(',');
	
  UART_OutString(ssid);          // Send WiFi SSID to ESP8266
  UART_OutChar(',');
  
  UART_OutString(pass);          // Send WiFi Password to ESP8266
  UART_OutChar(',');      
  
  UART_OutString(mqtt_broker);   // Send IP address of MQTT Broker   
  UART_OutChar(',');       
  
  UART_OutString(mqtt_port);     // Send MQTT port number  
  UART_OutChar(',');             // Extra comma needed for ESP8266 parser code
	UART_OutChar(0);
